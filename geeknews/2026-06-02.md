...
## 02:29 — NVIDIA, RTX Spark 공개
**source**: geeknews · **tags**: #RTX #AI #NVIDIA #최대 #GB
**url**: https://news.hada.io/topic?id=30102
...
- NVIDIA AI와 RTX 그래픽을 **하나의 칩**에 융합
- 슬림한 RTX 노트북과 소형·초고효율 데스크톱에서 **창작, AI 개발, 게이밍** 구현
- ### 핵심 사양

  - 최대 **6,144 코어 Blackwell RTX GPU**
  - 최대 **20 코어** 초고효율 CPU
  - 최대 **1 Petaflop FP4 AI** 성능
  - 최대 **128GB 통합 메모리**
- ### 주요 특징

  - **CUDA**가 네이티브로 실행되어 에이전트·AI 구동
  - 역대 가장 전력 효율적인 RTX 칩으로 **올데이 배터리**와 초슬림 섀시
  - 수백 개 창작 앱·AI 도구가 RTX 및 **NVIDIA Studio**로 가속
  - 레이트레이싱, 전체 **DLSS** 제품군, NVIDIA Reflex, G-SYNC 게임 지원
- ### AI 에이전트

  - 작업 실행·에셋 생성·코드 작성을 **온디맨드**로 수행하는 에이전트가 사용자와 함께 동작
  - 사용자가 목표를 설정하면 기기가 처리하는 **팀메이트형 PC**로 전환
- ### 사용자별 활용

  - Creators: **FP4 Tensor Cores**·통합 메모리, RT Cores·DLSS 실시간 3D 렌더, **4:2:2 하드웨어 인코딩/디코딩**, AV1 인코더와 NVIDIA Broadcast
  - Developers: 동일 **NVIDIA CUDA 스택**으로 개발·프로토타이핑, 최대 128GB로 로컬 파인튜닝·인퍼런스
  - Gamers: 레이트레이싱 조명, 전체 **DLSS**, NVIDIA Reflex
- ### 탑재 제품

  - 노트북: ASUS ProArt P16, Dell XPS 16, HP OmniBook X 14, Lenovo Yoga Pro 9n, Microsoft Surface Laptop Ultra, MSI Prestige N16 Flip AI+
  - 데스크톱: Acer, Asus, Dell, Gigabyte, HP, Lenovo, MSI — **개인 AI 에이전트 24/7** 구동

### 관련기사 #1 [Nvidia debuts RTX Spark processor for Windows laptops, taking aim at Intel, AMD](https://finance.yahoo.com/markets/article/nvidia-debuts-rtx-spark-processor-for-windows-laptops-taking-aim-at-intel-amd-053000567.html)

- **Intel·AMD 정조준** 관점에서 보도, 발표 당일 **Intel 6%·AMD 5% 하락**, NVIDIA 4%·Microsoft 3% 상승
- 칩 구성·출시
  - Blackwell GPU + **Grace CPU** 결합, **올가을** 출시
  - 탑재 제조사로 ASUS, Dell, HP, Microsoft 등 명시
  - 소형 데스크톱 버전도 동시 제공, 대상은 AI 에이전트 사용자·창작자·게이머
- 메모리·가격
  - 일반 노트북 16GB 대비 **128GB**는 이례적이며, 동일 구성 최상위 MacBook Pro는 **$5,099** 수준
  - 가격 미공개, 첫 제품군은 **프리미엄** 겨냥, 저용량 메모리 **하위 버전**도 예정
- 호환성·게이밍
  - **Arm 기반 Grace** 구조라 대부분 **x86 소프트웨어**의 구동을 위해 Microsoft·개발자와 협업
  - 이미 **Qualcomm Arm 프로세서**가 Windows 노트북을 구동하나, Arm 게이밍에는 잠재적 문제
  - 주요 게임 개발사 전체와 **안티치트** 작동 협업, DLSS 지원
  - 벤치마크 미공개, Blackwell GPU는 **RTX 5070 노트북 GPU** 등급으로 언급
- 디자인·공급
  - 두께 **약 14mm**, HD 웹캠, 올데이 배터리 힌트
  - 글로벌 메모리 부족에도 **공급 제한은 없을 것**으로 전망
- 함께 발표된 Vera CPU
  - 데이터센터용 **Vera CPU** 풀 생산 진입, Vera 전용 랙 서버 올가을 제공
  - **Anthropic, OpenAI, SpaceXAI, ByteDance, CoreWeave, Oracle Cloud Infrastructure**가 도입 검토

### 관련기사 #2 [Nvidia Challenges Apple Silicon With New RTX Spark PC Chip](https://www.macrumors.com/2026/06/01/nvidia-challenges-apple-rtx-spark-pc-chip/)

- NVIDIA가 **소비자 PC 칩 시장 첫 진입**으로 Apple에 도전장을 던졌다는 프레임
- 자체적으로 **"가장 효율적인 PC 칩"** 으로 규정, **Apple M5와 정면충돌** 구도
- 칩 정체성
  - Apple 칩처럼 **Arm 기반**, Blackwell RTX GPU + Grace CPU 결합
  - 사실상 작년 출시된 **DGX Spark**의 **GB10 칩**과 동일 구성
- NVIDIA 제시 작업 성능
  - OptiX·DLSS로 **90GB 초대형 3D 씬** 렌더링
  - Blackwell 디코더로 **12K 4:2:2 영상** 편집
  - **120억 파라미터 LLM**을 100만 토큰 컨텍스트로 구동
  - 레이트레이싱·DLSS·Reflex 적용 시 **1440p·100fps 이상** AAA 게임
- 출시 규모·Surface Laptop Ultra
  - 최종적으로 **노트북 약 30종·데스크톱 10종 이상**에 탑재 (Asus, HP, MSI, Lenovo, Dell)
  - 첫 탑재 기기 중 **15인치 Surface Laptop Ultra**, 미니-LED 터치스크린, 역대 최대 햅틱 터치패드, HDMI·USB-C·USB-A·SD·헤드폰 포트
  - 최대 128GB로 **120억 파라미터 모델 로컬 구동**, Microsoft가 역대 최강 Surface로 소개, 올해 후반 출시·가격 미정
- 독자 반응
  - 백그라운드 상시 작동 AI 에이전트 "**teammate**" 콘셉트에 거부감·냉소 다수
  - "결국 Windows 구동", "약 6년 만의 경쟁"과 함께 Apple이 **업계 방향성을 선도**했다는 평가도 제기

---

## 02:50 — Microsoft, NVIDIA 기반 Surface Laptop Ultra로 MacBook Pro 경쟁 제품을 만듦
**source**: geeknews · **tags**: #RTX #성능 #Surface #Windows #메모리
**url**: https://news.hada.io/topic?id=30104
...
- 색상은 Platinum과 Nightfall로 제공됨
- 15인치 mini-LED PixelSense Ultra 터치스크린은 2880×1920 해상도, 262ppi, 최대 2,000니트 피크 HDR 밝기를 제공함
- Surface 노트북 중 가장 큰 햅틱 터치패드를 탑재함
- 포트는 풀사이즈 HDMI, USB-C, USB-A, 전용 SD 카드 리더, 표준 헤드폰 잭으로 구성됨
- 플랫폼은 NVIDIA RTX Spark이며, MediaTek과 공동 개발한 20코어 NVIDIA Grace CPU와 Blackwell RTX GPU를 결합함
- GPU는 최대 6,144 CUDA 코어, 5세대 Tensor Cores, FP4 정밀도를 갖춤
- CPU와 GPU는 NVLink-C2C 칩 간 인터커넥트로 연결됨
- 최대 128GB 통합 메모리는 작업 부하에 따라 CPU와 GPU 사이에서 동적으로 배분됨
- 통합 메모리와 전체 CUDA 지원으로 1,200억 파라미터 AI 모델을 클라우드 없이 로컬에서 실행할 수 있음
- AI 연산 성능은 1페타플롭이며, 대형 장면 처리와 컴파일 작업을 겨냥함
- 수리 가이드와 교체 부품을 구매할 수 있고, SSD는 교체 가능함

## Windows 11 on Arm 최적화

- Microsoft는 새 실리콘 아키텍처 성능을 끌어내기 위해 Windows 11을 조정함
- 새 작업 부하 프로필 스케줄링 시스템은 20개 프로세서 코어 전체로 작업을 효율적으로 확장하도록 설계됨
- Microsoft Power and Thermal Framework는 새 플랫폼에서 전력 공급을 표준화해 강한 창작 작업 중에도 발열과 배터리 수명을 관리함
- 통합 메모리 구성에 맞춰 Windows는 GPU가 접근할 수 있는 전체 시스템 메모리 한도를 더 높고 지능적으로 관리함
- 공유 메모리 영역의 페이지 크기 관리도 더 효율적으로 바뀌어 무거운 렌더링 작업을 하는 개발자에게 유연성을 제공함
- NVIDIA는 Windows에 OpenShell runtime을 가져오며, Microsoft가 설계한 새 보안·격리 프리미티브를 활용함
- OpenShell의 격리 기능은 Hermes와 OpenClaw 같은 로컬 에이전트를 샌드박스화해 핵심 운영체제에 간섭하지 못하게 함
- Prism 에뮬레이션 계층은 새 마이크로아키텍처에 맞춰 최적화됐고, 실리콘 성능과 AVX·AVX2 명령어 확장을 활용해 기존 x86 앱을 에뮬레이션에서 부드럽게 실행함

## RTX Spark 생태계와 성능 목표

- RTX Spark 플랫폼은 Windows on Arm에서 성능 도약을 겨냥한 커스텀 실리콘으로 구성됨
- 성능 목표는 12K 비디오 편집, 초대형 3D 장면 렌더링, 1440p 해상도에서 초당 100프레임을 넘는 AAA 게임 실행까지 포함함
- Epic과 BattlEye의 네이티브 안티치트 솔루션은 RTX Spark 플랫폼을 완전히 지원함
- Riot Games는 League of Legends와 Valorant를 이 아키텍처에 네이티브로 가져오며, KRAFTON은 PUBG Battlegrounds를 가져옴
- NVIDIA는 Adobe와 함께 Premiere와 Photoshop을 RTX Spark 플랫폼에 맞춰 재설계하고 있음
- 업데이트된 Adobe 창작 도구는 통합 메모리와 TensorRT 소프트웨어를 직접 활용해 색 보정과 편집에서 실시간 성능 향상을 제공하는 방향을 택함
- Qualcomm이 Windows on Arm 생태계를 되살린 뒤, RTX Spark는 네이티브 CUDA 지원과 GPU 성능으로 더 높은 성능 계층을 겨냥함

## 가격과 다른 RTX Spark 기기

- Surface Laptop Ultra는 최대 128GB 메모리와 NVIDIA와의 새로운 칩 파트너십 때문에 프리미엄 가격대가 예상됨
- 글로벌 RAM 공급 제약도 출시 가격을 높이는 요인으로 거론됨
- Microsoft가 최근 [$1,300 8GB RAM Surface Laptop](https://www.windowslatest.com/2026/05/21/microsoft-pushed-16gb-ram-as-must-have-for-windows-11-for-years-now-sells-an-8gb-surface-laptop-for-1299/)을 발표한 점과 비교하면, Surface Laptop Ultra는 대부분의 전문가가 고려하기 어려울 만큼 비쌀 가능성이 큼
- NVIDIA RTX Spark 기반 기기는 Surface Laptop Ultra 외에도 ASUS ProArt P16·P14, Dell XPS 16 Creator Edition, HP OmniBook Ultra 16·OmniBook X 14, Lenovo Yoga Pro 9n, MSI Prestige N16 Flip AI+로 확장됨
- 최신 MacBook Pro 모델과 Surface Laptop Ultra의 실제 비교가 올해 하드웨어 경쟁의 주요 관전 포인트가 됨

---

## 03:00 — Nvidia Cosmos 3
**source**: geeknews · **tags**: #Cosmos #모델 #생성 #추론 #물리
**url**: https://news.hada.io/topic?id=30105
...
## 03:10 — 설계상 불법: 생성형 AI의 인권 비용 드러내기
**source**: geeknews · **tags**: #시스템 #생성 #AI #불법 #스크래핑
**url**: https://news.hada.io/topic?id=30106
...
- 불법 웹 스크래핑에 기반한 독립형 생성형 AI 시스템은 설계, 개발, 배포 단계에서 국제인권법(IHRL) 및 관련 기준과 충돌함
- Amnesty International은 이러한 시스템이 설계상 대규모 프라이버시 침해에 의존하며, 국제인권법과 근본적으로 양립할 수 없다고 봄

## 인권 침해 메커니즘

- 생성형 AI 기술은 정교한 자동화와 효율성을 약속함
- 그러나 데이터 수집과 모델 학습 관행은 프라이버시 권리 남용에 의존함
- 이러한 관행은 차별을 가능하게 하고, 표현의 자유와 사상의 자유를 위협함

## 결론과 요구

- Amnesty International은 불법 웹 스크래핑 기반 독립형 생성형 AI 시스템의 금지를 요구함
- 영문 보고서는 [Download PDF](https://www.amnesty.org/en/wp-content/uploads/2026/05/POL4009962026ENGLISH.pdf)에서 내려받을 수 있음

---

## 03:40 — CS336: 처음부터 만드는 언어 모델링
**source**: geeknews · **tags**: #모델 #학습 #과제 #구현 #언어
**url**: https://news.hada.io/topic?id=30107
...
- 2026년 3월 28일 기준 단일 B200 GPU 공개 가격은 [Modal](https://modal.com) $6.25/시간, [Lambda Labs](https://lambda.ai) $6.69/시간, [RunPod](https://www.runpod.io) $4.99/시간, [Nebius](https://nebius.com/) $5.50/시간 또는 선점형 $3.05/시간, [Together](https://www.together.ai/) $7.49/시간이다
...
- 강의는 월·수 3:00–4:20pm Skilling Auditorium에서 진행하며, 녹화본은 [YouTube playlist](https://www.youtube.com/watch?v=JuoVZkPBiKk&list=PLoROMvodv4rMqXOcazWaTUHhq-yembLCV)에 제공한다
...
## 04:51 — Ask GN: GeekNews 의 favorite 보기가 사라진거 같습니다.
**source**: geeknews · **tags**: #favorite #추가 #GeekNews #개별 #게시
**url**: https://news.hada.io/topic?id=30109

GeekNews 의 개별 게시글들을 보면 favorite 추가가 가능합니다.

그런데 자기 닉네임 클릭해서 확인되는 <https://news.hada.io/@{:name>} 페이지에서 어느순간 favorite 보는 메뉴가 사라졌습니다.

자신이 추가한 favorite 리스트를 볼려면 어떻게 해야 하나요?

---

## 04:51 — MorphOS 3.20 공개 — Mirari 지원, SFS2 도입, Radeon/TinyGL 대규모 업데이트
**source**: geeknews · **tags**: #지원 #개선 #그래픽 #MorphOS #플랫폼
**url**: https://news.hada.io/topic?id=30108

## MorphOS 3.20 공개 — Mirari 플랫폼 지원, SFS2 도입, Radeon/TinyGL 대규모 업데이트
...
## 05:32 — Show GN: Relivio: 배포 직후 15분을 STABLE/WATCH/RISK로 정리하는 도구
**source**: geeknews · **tags**: #배포 #직후 #verdict #판단 #Relivio
**url**: https://news.hada.io/topic?id=30110
...
## 05:52 — Show GN: Cadenza: .NET 10의 파일 기반 앱의 효용성을 극대화하는 미니멀 SDK
**source**: geeknews · **tags**: #Cadenza #AI #에이전트 #Ca #API
**url**: https://news.hada.io/topic?id=30112
...
## 05:52 — Show GN: Velith — AI 기반 장문 콘텐츠 제작 파이프라인
**source**: geeknews · **tags**: #에이전트 #문서 #정리 #생성 #book
**url**: https://news.hada.io/topic?id=30111
...
## 06:22 — MorphOS 3.20 공개 — Mirari 지원, SFS2 도입, Radeon/TinyGL 대규모 업데이트
**source**: geeknews · **tags**: #지원 #개선 #그래픽 #파일 #기능
**url**: https://news.hada.io/topic?id=30108
...
## 06:42 — 스탠퍼드 CS336을 위한 AI 에이전트 지침
**source**: geeknews · **tags**: #학생 #코드 #구현 #AI #과제
**url**: https://news.hada.io/topic?id=30113
...
## 07:33 — MiniMax-M3 데뷔, 주요 벤치마크 성능에서 GPT-5.5와 Gemini 3.1 Pro를 능가하며 비용은 단 5-10% 수준
**source**: geeknews · **tags**: #비용 #미니맥스 #모델 #오픈 #기업
...
## 09:04 — Show GN: AgentDir - 에이전트를 위한 mkdir - 에이전트 성능 극대화를 위한 read-only 가상 파일 시스템
**source**: geeknews · **tags**: #파일 #폴더 #구조 #코드 #정리
**url**: https://news.hada.io/topic?id=30115
...
## 11:36 — 주식시장은 Anthropic, SpaceX, OpenAI를 삼킬 수 있을까?
**source**: geeknews · **tags**: #Anthropic #OpenAI #SpaceX #최대 #조달
**url**: https://news.hada.io/topic?id=30116

- **SpaceX**, Anthropic, OpenAI의 상장 계획은 사상 최대 규모의 주식시장 데뷔가 될 가능성이 있음
- **SpaceX**는 6월 11일 투자자들로부터 750억 달러 조달을 희망하며, 발행 주식은 다음 날 Nasdaq에서 거래를 시작할 예정임
- **Anthropic**은 6월 1일 기업공개 초안 서류를 제출했고, 경쟁사 OpenAI도 곧 제출할 것으로 예상됨
- Anthropic과 **OpenAI**는 각각 최대 600억 달러 조달을 추진한다는 소문이 있음
- 세 건의 **기가 IPO**는 몇 달 안에 미국 상장기업 시장가치에 최대 4조 달러를 더할 수 있음

---

## 초대형 상장 일정

- SpaceX는 6월 11일 투자자들로부터 750억 달러 조달을 희망함
- SpaceX가 발행하는 주식은 다음 날 Nasdaq 거래소에서 거래를 시작할 예정임
- Anthropic은 6월 1일 IPO 초안 서류를 제출함
- OpenAI는 Anthropic과 경쟁하는 기업으로, 곧 IPO 서류를 제출할 것으로 예상됨

## 조달 규모와 시장 영향

- Anthropic과 OpenAI는 각각 최대 600억 달러 조달을 모색한다는 소문이 있음
- SpaceX, Anthropic, OpenAI의 세 건의 기가 IPO는 함께 미국 상장기업 시장가치에 최대 4조 달러를 더할 수 있음
- 이 변화는 몇 달 안에 발생할 수 있는 규모로 제시됨

---

## 11:46 — 주식시장은 Anthropic, SpaceX, OpenAI를 삼킬 수 있을까?
**source**: geeknews · **tags**: #시장 #기업 #IPO #주식 #SpaceX
**url**: https://news.hada.io/topic?id=30116
...
- SpaceX는 6월 11일 투자자로부터 **$75bn 조달**을 희망하며, 발행 주식은 다음 날 Nasdaq에서 거래 개시 예정
- Anthropic은 6월 1일 IPO 초안 서류를 제출했고, 경쟁사 OpenAI도 곧 제출할 것으로 예상
  - Anthropic과 OpenAI는 각각 최대 **$60bn**을 노린다는 소문
...
- SpaceX·Anthropic·OpenAI는 합쳐 **$200bn 가량**을 목표
...
- 대부분의 지수는 공개 거래에 풀린 주식 가치(**free float**)에 비례해 기업을 가중
  ...
  - **NASDAQ 100**은 예외로, free float의 최대 3배까지 가중하도록 규칙 변경(Musk 영입 의도로 보임), 그래도 $40조 규모 지수에서 SpaceX 초기 비중은 약 0.5%

## 락업(Lock-up)과 점진적 시장 유입

- 미국 상장 테크 대기업은 한 곳을 제외하고 모두 free float이 85% 이상, 최저는 2012년 상장한 **Meta**로 창업자 Mark Zuckerberg가 13% 보유
...
- University of Florida의 **Jay Ritter**가 1980~2024년 상장 종목의 IPO 이후 수익률을 연구
  - 평균적으로 첫 거래일 이후 3년간 시장 대비 **20%포인트** 낮은 수익률 기록
  - 매출의 40배 이상으로 평가된 기업은 **58%포인트** 부진
  - $1.75조 가치의 SpaceX는 매출의 **90배 이상**에서 거래 시작 예정

## 강세장 정점 신호와 시장 조정 위험

- 블록버스터 IPO는 종종 강세장이 정점에 가까워진 신호로 해석(기업이 최고가에 주식을 팔려 하기 때문)
  - 직전 상장 급증기인 **2020~2021년**은 약세장 직전, 1990년대 후반과 2008년 이전 호황기는 더 큰 침체로 이어짐
- 세 거대 기업이 부진하면 시장 **조정(correction)** 을 촉발할 수 있음
  - 미국 최대 AI 관련 상장사 10곳이 이미 S&P 500 가치의 **5분의 2** 차지
  - SpaceX 단독 악재는 추적 펀드에 큰 타격이 아니나, **AI 전반의 악재**는 확실히 타격
  - 시가총액이 아닌 동일 가중 펀드가 일부 방어책이나, 현재로선 시장에 역베팅하는 것으로 패시브 투자의 반대

## 자본조달 환경의 전환

- 더 넓은 우려는 기가-IPO가 신규·기존 테크 대기업 모두의 추가 자본조달을 예고하는 점
- Elm Wealth의 **Victor Haghani**에 따르면 수년간 자본은 풍부하고 주식은 점점 희소
  - 테크 대기업은 막대한 현금으로 신주 발행 대신 **자사주 매입**을 진행, 화이트칼라 노동자는 퇴직 저축을 시장에 투입, 이로 인해 주가 상승
...
## 12:06 — RGB 값을 255로 정규화해야 할까, 256으로 정규화해야 할까?
**source**: geeknews · **tags**: #방식 #처리 #이미지 #표준 #구간
...
표준 방식은 정수 0을 0.0, 255를 1.0에 매핑하며 [GPU의 UNORM-to-float 변환](https://microsoft.github.io/DirectX-Specs/d3d/archive/D3D11_3_FunctionalSpec.htm#UNORMtoFLOAT) 방식과 같음

대안 방식은 0을 `0.5 / 256 = 0.001953125`에 매핑하므로, 검은 픽셀을 감지하려면 이 상수를 알아야 함
...
예를 들어 부동소수점 색상에서 `0.005`를 빼면 표준 방식의 검정은 음수가 되지만, 최종 결과는 여전히 정수 0이 됨
...
예를 들어 `128 / 255.0 ≈ 0.501961`이지만 `128 / 256.0 = 0.5`임

이 차이는 32비트 부동소수점의 23비트 가수에서 최하위 비트 수준의 반올림 오차이며, 크기는 `2^-23`보다 작음
...
Andrew Kesler의 2015년 글 [“Converting Color Depth”](http://eastfarthing.com/blog/2015-12-19-color/)는 이 방식이 디더링에서 노이즈를 더할 때 경계 처리를 덜 신경 쓰게 만든다고 봄

반대로 표준 방식의 양끝 구간은 노이즈 분포를 일관되게 유지하려면 [주의 깊은 처리](https://computergraphics.stackexchange.com/questions/5904/whats-a-proper-way-to-clamp-dither-noise/8777#8777)가 필요함

## 양자화 관점

두 방식은 균일 스칼라 양자화기(uniform scalar quantizer)로 볼 수 있음

[Wikipedia의 quantization 설명](https://en.wikipedia.org/wiki/Quantization_(signal_processing))은 signed input data의 균일 양자화기를 주로 mid-riser와 mid-tread로 나눔
...
표준 방식은 `L=255`를 쓰는 mid-tread 형태이고, 대안 방식은 `L=256`을 쓰는 mid-riser 형태임
...
StackOverflow 사용자 [Peter Mudrievskij의 계산](https://stackoverflow.com/a/79805625)에 따르면 평균 절대 오차는 255 나누기에서 `1 / 1020`, 256 나누기에서 `1 / 1024`임
...
## 15:39 — 소셜 미디어 연령 확인, 자유로운 인터넷의 종말의 시작인가?
**source**: geeknews · **tags**: #확인 #신원 #제한 #연령 #도입
**url**: https://news.hada.io/topic?id=30118
...
- 현재 도입되는 방식은 개별 웹사이트와 서비스가 자체 구현하며, 2025년 가을 Discord 해킹으로 [사용자 70,000명의 ID 문서가 노출](https://www.bbc.com/news/articles/c8jmzd972leo)되면서 품질 차이가 드러남
...
- 영국에서는 당국이 “grossly offensive”로 분류한 온라인 게시물 때문에 [매일 30명이 체포](https://reclaimthenet.org/britains-free-speech-crisis-and-the-bill-that-would-fix-it)되고, 독일에서는 정치인을 온라인에서 모욕했다는 이유로 [주거지 압수수색](https://www.yahoo.com/news/german-police-launch-nationwide-operation-061636979.html)이 이뤄짐
- 미국 당국은 ICE 항의 계정 뒤의 신원을 밝히도록 기술 기업을 압박하고, 캐나다는 2022년 트럭 운전사 시위 때 소셜 미디어로 시위 참여자를 식별해 재정 지원자의 [은행 계좌를 동결](https://www.cbc.ca/news/politics/ottawa-protests-frozen-bank-accounts-1.6355396)함

## 빠르게 확산되는 소셜 미디어 연령 제한

- [Australia](https://www.bbc.com/news/articles/cwyp9d3ddqyo)는 16세 미만 이용자 대상 소셜 미디어 연령 제한을 도입했고, [Indonesia](https://www.reuters.com/business/media-telecom/indonesias-social-media-curbs-kids-set-saturday-few-know-how-it-will-work-2026-03-27/)와 [Brazil](https://www.techradar.com/vpn/vpn-privacy-security/vpn-interest-spikes-in-brazil-as-mandatory-age-verification-law-takes-effect)도 같은 방향으로 움직임
- [Denmark](https://apnews.com/article/denmark-social-media-ban-children-7862d2a8cc590b4969c8931a01adc7f4), [Portugal](https://www.reuters.com/world/europe/portugal-approves-restrictions-social-media-access-children-2026-02-12/), [Malaysia](https://fortune.com/2025/12/09/australia-will-start-banning-kids-from-social-media-this-week-and-malaysia-is-getting-ready-to-do-the-same/)는 제한을 승인했지만 아직 시행하지 않았고, [France](https://news.sky.com/story/the-countries-that-have-social-media-bans-or-are-planning-to-implement-one-13526116)는 합의 후 세부 사항을 논의 중임
- [Spain](https://www.bbc.com/news/articles/c5y2nddvmryo)과 [Turkey](https://www.reuters.com/technology/turkeys-ruling-party-submits-bill-ban-social-media-under-15s-2026-03-04/)에는 제안이 올라와 있고, [Germany](https://www.reuters.com/business/media-telecom/germanys-ruling-party-backs-social-media-curbs-children-2026-02-21/)의 주요 정당들은 연령 제한 도입에 동의했으며, [Sweden](https://regeringen.se/pressmeddelanden/2025/10/regeringen-tillsatter-utredning-om-aldersgrans-for-sociala-medier/)은 조사 단계에 있음
- 2026년 4월 European Commission은 [EU age verification app](https://x.com/vonderleyen/status/2044340323120193595)을 공개했고, 한 달 뒤 Ursula von der Leyen은 [EU 전역 연령 제한](https://www.bbc.com/news/articles/c9q3x19ddl7o) 계획을 내놓음
- [United States](https://www.cnbc.com/2026/03/08/social-media-child-safety-internet-ai-surveillance.html)에서는 전체 주의 절반이 부적절한 콘텐츠나 소셜 미디어에 대한 연령 제한 법안을 계류 중이거나 이미 도입함: {p:50}

## 우회 차단이 VPN·앱스토어·운영체제로 확장

- 국가 단위 제한은 VPN, 가상 전화번호, eSIM, Tor, 전용 서비스로 디지털 위치를 바꿔 우회할 수 있음
- 영국 House of Lords는 2026년 초 Children’s Wellbeing and Schools Bill 관련 [수정안](https://bills.parliament.uk/bills/3909/stages/20215/amendments/10027478)을 House of Commons에 보내 VPN 서비스 이용에 18세 연령 제한을 두려 했고, House of Commons는 이를 네 차례 거부함
- House of Commons는 별도 [제안](https://www.eff.org/deeplinks/2026/03/uk-politicians-continue-miss-point-latest-social-media-ban-proposal)을 통과시켜 정부가 제한을 2차 입법으로 도입할 권한을 갖게 했고, 이 내용은 [법이 됨](https://www.legislation.gov.uk/ukpga/2026/21/section/70/enacted)
- 영국 정부는 16세 미만 아동에 대한 소셜 미디어 제한을 도입하겠다고 [확인](https://www.bbc.com/news/articles/c5y7d2zx63jo)했고, VPN 사용에 대한 [신원 확인 도입 가능성](https://www.gov.uk/government/news/pm-no-platform-gets-a-free-pass-government-takes-action-to-keep-children-safe-online)도 내비침
- 프랑스 AI·디지털 담당 장관 Anne Le Hénanff는 “[VPNs are the next topic on my list](https://www.techradar.com/vpn/vpn-privacy-security/vpns-are-next-on-my-list-france-set-to-evaluate-vpn-use-following-social-media-ban-for-under-15s)”라고 말했고, Utah는 VPN으로 제한을 우회하는 행위를 [불법으로 만드는 법](https://www.eff.org/deeplinks/2026/04/utahs-new-law-regulating-vpns-goes-effect-next-week)을 도입함
- EU에서는 [Going Dark](https://mullvad.net/why-privacy-matters/going-dark)와 [연령 확인 관련 논의](https://www.europarl.europa.eu/thinktank/en/document/EPRS_ATA(2026)782618)에서 VPN 제한이 다뤄졌고, EU Commissioner Henna Virkkunen은 연령 확인이 우회되지 않도록 다음 단계를 봐야 한다고 말함
...
- Australia, Brazil, South Korea, Singapore, 미국 여러 주에서는 Apple이 성인 콘텐츠가 있는 앱 접근을 제한하기 위해 [App Store 수준 신원 확인](https://reclaimthenet.org/apple-18-plus-app-blocks-age-verification-expansion)을 도입하기 시작함
- 앱스토어 수준 신원 확인은 앱 접근만 규제하므로, 여러 국가는 특정 웹사이트 접근을 OS에서 직접 막기 위해 [운영체제 수준 신원 확인](https://reclaimthenet.org/apple-forces-uk-iphone-age-checks-in-ios-26-4)을 요구하기 시작함
...
- [California](https://reclaimthenet.org/california-ab-1043-os-age-verification-law)는 2027년 1월부터 운영체제 수준 신원 확인을 요구하는 유사 법을 통과시켰고, 초기에는 오픈소스 시스템도 대상이었지만 나중에 [제외](https://reclaimthenet.org/california-wants-an-age-tracker-for-the-whole-web)했으며 웹브라우저와 웹사이트를 범위에 넣음
- [Colorado](https://www.pcmag.com/news/colorado-lawmakers-push-for-age-verification-at-the-operating-system-level), [New York](https://www.theregister.com/2026/03/06/os_age_verification/)에도 유사 제안이 있고, 2026년 4월에는 미국 전역에 [OS 수준 신원 확인](https://www.congress.gov/bill/119th-congress/house-bill/8250/all-info)을 요구하는 연방 제안이 나옴
...
- 영국 Online Safety Act는 [Wikipedia 접근을 제한](https://www.bbc.com/news/articles/cjr11qqvvwlo)할 수 있으며, 연령 확인이 점차 확장되면 청소년이 의미 있는 디지털 만남의 공간에서 배제될 수 있음

## 아동 안전 명분과 감시 인프라

- 아동의 “안전”은 정보기관과 당국이 대량 감시를 도입할 때 반복적으로 사용한 명분임
- 미국 Kids Online Safety Act(KOSA)는 아동 보호를 이유로 [신원 확인 도입](https://theintercept.com/2026/03/05/kosa-online-age-verification-free-speech-privacy/)을 시도함
- 영국 Online Safety Act에서는 정치권이 종단간 암호화 통신 [스캔 승인](https://www.computerweekly.com/news/366636751/Privacy-will-be-under-unprecedented-attack-in-2026)을 반복적으로 시도함
...
## 15:49 — OpenAI 프런티어 모델과 Codex가 이제 AWS에서 제공됨
**source**: geeknews · **tags**: #AWS #OpenAI #제공 #Codex #보안
**url**: https://news.hada.io/topic?id=30119
...
## 16:40 — Chipotlai Max - Chipotle Pepper AI를 기본 모델로 쓰는 밈 AI 코딩 에이전트
**source**: geeknews · **tags**: #Chipotle #프록시 #챗봇 #Pepper #리버스
**url**: https://news.hada.io/topic?id=30120
...
## 17:51 — Show GN: Hunchi - 초기 프로젝트의 다음 마일스톤을 YES/NO로 예측하는 서비스
**source**: geeknews · **tags**: #프로젝트 #초기 #서비스 #다음 #예측
**url**: https://news.hada.io/topic?id=30121

안녕하세요. 초기 프로젝트를 만드는 분들을 위한 Hunchi라는 서비스를 만들었습니다.

만들게 된 계기는 다음과 같습니다.

인디/초기 프로젝트는 한 번 출시하거나 공유해도 관심이 오래 이어지기 어렵고, SNS 계정을 이전부터 잘 키워오지 않은 이상 “다음 목표를 계속 지켜봐주는 사람”을 만들기가 쉽지 않다고 느꼈습니다.

Hunchi는 초기 프로젝트의 다음 마일스톤을 YES/NO 예측 형태로 올리고, 사람들이 그 진행을 가볍게 따라가 볼 수 있게 만드는 서비스입니다.

예를 들면:

- 이번 주 안에 기능 X를 출시할 수 있을까?
- 이번 달 안에 월 방문자 X명을 넘길 수 있을까?
- 다음 공개 업데이트까지 목표 X를 달성할 수 있을까?

돈을 거는 베팅 서비스는 아니고, 예측/점수/랭킹을 통해 초기 프로젝트의 진행을 조금 더 액티브하게 만들어보려는 실험입니다.

현재 막 런칭했고, 첫 외부 빌더 프로젝트를 찾고 있습니다. 무언가 만들고 계신 빌더라면, 가볍게 이용해보세요.

글로벌 대상으로 생각하고 만든 서비스라 현재 등록된 제 프로젝트는 영어로 되어 있습니다. 다만 한국어로 올리셔도 상관 없습니다.

궁금한 점:

- 이런 “마일스톤 예측”이 초기 프로젝트를 계속 보게 만드는 데 실제로 도움이 될까요?
- 빌더 입장에서 프로젝트를 등록하려면 어떤 부분이 더 명확해야 할까요?

---

## 22:44 — DuckDuckGo, 트래픽 급증 속 ‘No-AI’ 검색 엔진에 더 쉽게 접근할 수 있게 함
**source**: geeknews · **tags**: #AI #검색 #DuckDuckGo #페이지 #확장
**url**: https://news.hada.io/topic?id=30124
...
## No-AI 검색을 기본값으로 설정하는 확장

- DuckDuckGo는 AI 없는 검색 페이지인 [noai.duckduckgo.com](http://noai.duckduckgo.com)을 기본 검색 엔진으로 설정하는 새 브라우저 확장을 출시함
- 확장은 현재 [Chrome](https://chromewebstore.google.com/detail/duckduckgo-no-ai-search/faoilnlkccdjdkpljainiiimmijofmpd)과 [Firefox](https://addons.mozilla.org/en-US/firefox/addon/duckduckgo-no-ai-search/)에서 사용할 수 있음
- 활성화하면 사용자는 AI 보조 답변과 채팅 프롬프트가 없는 DuckDuckGo의 AI-free 검색 페이지로 이동함
- DuckDuckGo 웹 브라우저로 전환한 사용자는 브라우저 기록을 지워도 AI 설정이 유지됨
- DuckDuckGo는 기존 DuckDuckGo Privacy Essentials 확장도 업데이트해 Chrome, Firefox, Edge, Opera에서 AI 검색 설정 제어를 제공할 예정임

## Google 검색 변화와 DuckDuckGo 트래픽 증가

- Google은 5월 개발자 콘퍼런스에서 검색 엔진을 AI 우선 방식으로 개편한다고 발표함
- Google 검색은 페이지 상단의 링크보다 AI 생성 검색 개요를 우선하고, 이 개요는 필요에 따라 시각화, 차트, 그래프, 미니 앱을 만드는 더 상호작용적인 경험으로 바뀜
- AI Overviews의 후속 질문은 사용자를 AI Mode 채팅 경험으로 이동시키며, 기존의 “10 blue links”는 AI 기반 기능 아래에 놓이는 형태가 됨
- DuckDuckGo는 Google 발표 이후 No-AI 검색 페이지 트래픽이 2026년 5월 28일 목요일 3배로 늘어 새 고점을 기록했고, 이후에도 증가세가 이어진다고 밝힘
- DuckDuckGo는 반AI 회사는 아니며, 여러 인기 모델에 접근할 수 있는 자체 AI 챗봇과 최신 모델, VPN, 신원 도용 복구, 개인정보 삭제 서비스를 제공하는 구독 플랜도 운영함

---

## 23:15 — Chipotlai Max - Chipotle Pepper AI를 기본 모델로 쓰는 밈 AI 코딩 에이전트
**source**: geeknews · **tags**: #Chipotle #Pepper #챗봇 #프록시 #지원
**url**: https://news.hada.io/topic?id=30120

- Chipotle의 고객 지원 챗봇 **Pepper**를 코딩 모델로 써서 OpenCode포크 위에서 무료 추론을 돌리는 **밈(meme) 프로젝트**
- Pepper가 LeetCode 문제 풀이, Python 작성, 링크드 리스트 뒤집기까지 처리하는 것이 발견되며 **대규모 바이럴** 되었음
- 이 챗봇은 Claude나 GPT가 아닌 **IPsoft Amelia** 기반이며, 백엔드를 리버스 엔지니어링해 **[OpenAI 호환 프록시](https://github.com/Gonzih/chipotle-llm-provider)** 로 공개한 것을 이용
  - 이 프록시는 로컬에서 구동되어 `http://localhost:3000/v1`로 OpenAI 호환 API를 공개하며 **API 키가 전혀 필요없음**
- **OpenCode(MIT)** 를 포크해 Pepper를 기본 모델로 하드코딩하고 Chipotle 브랜드 색상을 입힘
- Chipotle 프로덕션 지원 봇을 리버스 엔지니어링하므로 TOS 위반 가능성과 프록시 차단 위험을 안고 있어, **교육·밈 목적**으로만 쓰도록 명시
- Chipotle Pepper가 2026년 3월 패치되어 현재는 Home Depot, Lowe’s, Target, Starbucks, Walmart, McDonald’s 같은 다른 기업 챗봇 제공자 프록시를 찾고 있음

---

## 23:35 — Codex, 역할별 플러그인·Sites·주석으로 비개발자 업무까지 확장
**source**: geeknews · **tags**: #플러그인 #Codex #가능 #전환 #사이트
**url**: https://news.hada.io/topic?id=30125
...
- 매주 500만 명 이상이 Codex 사용, 개발 도구로 시작했으나 더 많은 종류의 업무로 유용성 확대
- 분석가·마케터·운영자·디자이너·연구자·투자자·뱅커 등 **비개발자**가 전체 사용자의 약 20%를 차지, 개발자보다 3배 이상 빠르게 증가
- 사내외 활용 사례
  - OpenAI 내부 비기술 팀은 내부 앱 구축, 경영 자료 준비, 대시보드 생성, 크리에이티브 브리프를 브랜드·디자인 제약에 맞는 결과물로 전환
  - Zapier는 Slack, Google Docs, Coda 등에서 지식을 끌어와 **포스트모템, 인시던트 대응 계획, 기능 티켓**으로 전환
  - NVIDIA 연구진은 연구 아이디어 발굴부터 머신러닝 인프라 스크립트 작성까지 **실험 워크플로** 가속에 활용

## 팀 방식에 맞춘 Codex — 역할별 플러그인

- 플러그인은 팀이 이미 사용하는 도구·컨텍스트·워크플로와 Codex를 연결, 코딩 불필요
- 각 플러그인은 관련 앱·스킬·지침·워크플로를 묶음 형태로 제공, 전체적으로 **62개 인기 앱과 110개 스킬** 포함
- ### 데이터 분석 플러그인

  - 분석가·비즈니스 팀이 데이터로 질문에 답하고, 핵심 지표 변동 원인을 설명하며 리포트·대시보드 생성
  - Snowflake, Databricks Genie, Hex, Tableau 등 도구 활용 (추가 예정)
- ### 크리에이티브 제작 플러그인

  - 마케팅·크리에이티브 팀이 브리프를 검토 가능한 에셋으로 전환, 캠페인 보드 생성 및 디스플레이 광고 변형 제작·개선
  - 제품 라이프스타일 컷이나 이커머스용 이미지 세트 제작, Figma, Canva, Shutterstock, Picsart, Fal 활용
- ### 세일즈 플러그인

  - 세일즈 팀이 고객 컨텍스트를 거래 진전 업무에 반영, 우선순위 계정·신호 탐색, 고객 미팅 준비, 후속 작업 완료
  - 고객 기록 업데이트, 클로즈 플랜 구축, 위험 거래 검토에 Salesforce, HubSpot, Slack, Outreach, Clay, Rox, Actively 활용
- ### 제품 디자인 플러그인

  - 초기 아이디어를 검토 가능한 **프로토타입**으로 전환, 제품 방향 탐색 및 사용자 플로우 점검
  - 라이브 URL로부터 프로토타이핑하고 정적 스크린샷을 인터랙티브하게 변환, Figma·Canva로 작업 이어가기 가능
- ### 퍼블릭 에쿼티 투자 플러그인

  - 투자자가 시장·기업 정보를 파악, 실적 검토·기업 비교·신호 추적 및 **투자 논거(thesis)** 의 강화·약화 여부 평가
  - Moody's, Daloopa, Datasite, FactSet, LSEG, S&P, PitchBook, Hebbia 정보 활용
- ### 투자은행 플러그인

  - 뱅커가 리서치·실사를 **고객 제출용 자료**로 전환, 피치 자료 준비 및 비교 기업·거래 분석
  - 신뢰할 수 있는 데이터로 실사를 권고안으로 전환
- 플러그인은 기본 상태로 즉시 작동, 팀이 워크플로에 맞게 조정하거나 자체 시스템·프로세스용 커스텀 플러그인을 구축·공유 가능
- 추가 역할별 플러그인 출시 예정 — Corporate Finance, Private Equity Investing, Marketing Strategy, Strategy Consulting, Legal 포함
- 파트너가 Codex 및 ChatGPT에 직접 플러그인을 만들고 배포하는 **개방형 생태계** 지향

## Sites로 작업물 공유

- Codex가 인터랙티브 호스팅 웹사이트·앱 생성 및 공유 지원
- 비즈니스·엔터프라이즈 고객 대상 프리뷰 제공부터 시작
- 아이디어·분석·계획을 대시보드, 플래너, 리뷰 워크스페이스, 프로젝트 보드, 갤러리, 경량 도구로 전환하는 **새로운 형태의 캔버스**
- 워크스페이스 내 누구와도 URL로 공유 가능, 팀이 함께 작업 탐색·의견 기여·진행 추적·의사결정을 수행하는 공유 공간 제공
- 구체적 활용 예시
  - 고객 리뷰용 사이트 요청 시 관련 제품 업데이트, 미해결 질문, 사용 추세, 다음 단계가 담긴 인터랙티브 웹페이지 생성
  - 재무 모델로부터 **시나리오 플래너** 구축, 리더가 문서 탭을 넘기는 대신 가정을 비교
  - 런칭 자료를 최신 메시징·마일스톤·담당자·결정사항을 찾을 수 있는 **살아있는 허브**로 전환하고 세부사항 변경 시 자동 최신화
- 사이트는 정적이지 않으며, 대형 프로젝트 진행 추적, 고객 서비스 담당자 안내, 팀 크리에이티브 브리프 저장소 역할 수행
- Vercel, Wix, Base44, Replit, Lovable, Figma, Webflow, Emergent 등 초기 파트너와 협력하며 사이트 파트너 생태계 구축 중

## 주석으로 작업물 개선 (Annotations)

- 개발자가 코드, Markdown 파일, Codex 생성 웹사이트를 다듬는 데 이미 사용하는 주석 기능을 문서·스프레드시트·슬라이드 등 생성 콘텐츠로 확대
- 수정하려는 정확한 지점을 지목하고 변경 사항을 전달하는 방식
  - 사이트의 내비게이션 바를 선택해 폰트 업데이트 요청
  - 투자 논거의 특정 주장을 강조해 출처 질의
  - 슬라이드의 차트를 표시해 더 명확한 라벨 요청
- Codex가 선택한 부분에 수정을 집중, 마음에 드는 부분을 재작업하거나 처음부터 다시 시작할 필요 없음
- 판단·피드백·반복이 필요한 **초안 이후 단계**에서 Codex 유용성 강화

## 이용 가능 범위 및 시작 방법

- 역할별 플러그인은 지원 지역에서 순차 출시, Codex 플러그인 디렉터리에서 설치 가능하며 Codex가 설정 및 커스터마이즈 지원
- 비즈니스·엔터프라이즈 워크스페이스에서는 관리자가 워크스페이스 설정에서 기반 앱 권한 제어 가능
- 사이트는 Codex 앱을 통해 비즈니스·엔터프라이즈 팀 대상 프리뷰로 순차 출시, 엔터프라이즈 관리자가 관리자 설정에서 활성화 가능
