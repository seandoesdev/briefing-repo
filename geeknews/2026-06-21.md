...
## 15:43 — Windows 11 새 Media Player, RAM 3.5배 더 쓰고 인기 동영상 코덱은 유료화
**source**: geeknews · **tags**: #Media #Player #Windows #코덱 #기본
**url**: https://news.hada.io/topic?id=30690

- Windows 11의 새 **Media Player**는 기본 미디어 앱으로 이동하는 과정에서 메모리 사용량과 코덱 과금이 함께 논란이 되고 있음
- Windows Latest 테스트 기준 유휴 상태 RAM 사용량은 새 플레이어가 약 **377MB**, 기존 Windows Media Player가 약 103MB로 약 3.5배 차이남
- 로컬 동영상 파일 시작 시간도 기존 약 2초에서 새 플레이어 약 3초로 늘어 **약 50% 증가**함
...
- Windows 11의 새 **Media Player**는 Groove Music과 클래식 Windows Media Player를 대체하는 기본 앱 역할을 맡음
...
- Windows 11 버전 **24H2**에서는 내장 **AC-3(Dolby Digital)** 코덱이 제거됨
...
- 클래식 Windows Media Player는 **선택적 구성 요소**로 계속 제공되지만, 기본 선택지는 새 Media Player 쪽으로 이동하고 있음
- 서드파티 플레이어를 써도 괜찮다면 **VLC** 같은 대안도 있음
  - VLC는 자체 코덱을 포함해 Microsoft의 유료 추가 기능에 의존하지 않음

---

## 16:04 — Alice는 기다림을 싫어합니다
**source**: geeknews · **tags**: #시간 #평균 #분포 #복구 #요청
**url**: https://news.hada.io/topic?id=30691
...
## 16:44 — 느린 호흡은 뇌 기능과 위험 행동을 조절함
**source**: geeknews · **tags**: #조건 #날숨 #보상 #선택 #호흡
**url**: https://news.hada.io/topic?id=30692
...
- 연구는 의도적 자율신경 조절이 인간의 **위험 의사결정**에 영향을 주는지 검증함
...
- 사전등록 가설은 긴 날숨이 위험 선택을 늘리고, 심장 부교감 활동을 높이며, 보상 관련 신경 표상을 바꾼다는 것이었음
  - 사전등록: [Open Science Framework](https://osf.io/4cbfz)

## 참가자와 위험 선택 과제

- 건강한 성인 49명을 모집했고, 8명이 전체 분석에서 제외되어 최종 행동 표본은 **41명**임
  - 여성 24명
...
- 부교감 활동 검증에는 **RespHRV**와 **RMSSD**가 사용됨
...
- 따라서 긴 날숨 효과는 전반적 교감 각성 변화보다 **심장 부교감 활동**의 선택적 증가로 해석됨

## 위험 수락 증가와 보상 민감도

- 위험 선택은 logit link를 쓰는 **GLMM**으로 분석됨
...
- expected value 기반 GLMM도 긴 날숨에서 **EV 민감도**가 증가했음을 보임
...
- 긴 날숨에서 **ΔRMSSD**가 큰 참가자는 선택 중 보상 관련 활동이 더 강했음
...
- 긴 날숨은 자율신경 상태, 가치 관련 뇌 활동, 위험 선택의 체계적 변화와 연결되는 **neurovisceral pathway**의 증거로 정리됨
...
## 17:45 — SMPTE, 표준 라이브러리 무료 공개
**source**: geeknews · **tags**: #표준 #SMPTE #Standards #공개 #접근
...
- 산업은 **IP 기반 워크플로**, AI authenticity, content provenance 같은 전환적 변화에 직면해 있음
- 회원, 파트너, 글로벌 표준 커뮤니티의 의견을 반영한 결과, 미디어의 미래에는 상호운용성이 필수라는 판단에 도달함
- 표준 접근을 여는 조치는 다음 세대 미디어 기술이 더 강하고 접근 가능한 기반 위에 구축되도록 하기 위한 움직임임

## 표준 개발·출판 프로세스 현대화

- 오픈 액세스 Standards Library 전환은 SMPTE의 **표준 개발·출판 현대화** 작업의 일부임
- 최근 이니셔티브에는 다음이 포함됨
  - 버전 관리, 이슈 추적, 자동화를 위한 **GitHub 기반 워크플로** 도입
  - 구조화된 **HTML 기반 저작**으로 전환
  - 문서 생성, 검토, 검증, 릴리스를 간소화하는 통합 출판 파이프라인 구현
...
## 17:45 — 도서관에서 재봉틀을 빌리는 핀란드 모델
**source**: geeknews · **tags**: #도서관 #핀란드 #공공 #공간 #서비스
**url**: https://news.hada.io/topic?id=30693
...
- 헬싱키 중앙도서관 **Oodi**는 아침 8시 개장 전부터 사람들이 기다리고, 점심 무렵에는 빈자리를 찾기 어려울 정도로 붐빔
- 방문자들은 노트북으로 공부하고, 아이와 책을 읽고, 뜨개질을 배우고, 음악 부스에서 색소폰을 녹음하고, 핀란드어 회화 수업에 참여하고, 농구공을 빌려 야외 코트에서 사용함
- Oodi는 2019년 세계 최고의 신축 도서관으로 선정됐고, 공공도서관이 줄어드는 다른 국가들과 뚜렷하게 대비됨
  - 미국에서는 2008~2019년 공공도서관 766곳이 문을 닫음
  - 영국에서는 2016~2023년 지자체 운영 도서관 180곳 이상이 폐쇄되거나 자원봉사 단체로 넘어감
  - 핀란드는 도서관을 확장하며 공공 재정 기반의 **커뮤니티 서비스 센터**로 바꾸고 있음

## 공간과 도구를 함께 빌리는 방식

- 핀란드는 인구 560만 명에 700개 넘는 도서관을 두고, 팟캐스트 스튜디오, 3D 프린팅, 테니스 라켓, 수영장 이용권 등 다양한 서비스를 제공함
- 헬싱키 도서관에서 책 다음으로 많이 빌리는 것은 무료로 사전 예약 가능한 **공간**임
  - 모임, 공부, 정치 토론, 음악 제작 등에 사용됨
  - 휴대 가능한 물품 중에는 보드게임과 콘솔 게임이 가장 많이 빌려짐
- 이런 대여 문화는 농기계를 공유하던 핀란드의 농촌 과거와 연결됨
  - 도시 거주자는 작은 집에 살고, 재봉틀을 1년에 한 번만 필요로 할 수 있음
  - 세금으로 운영되는 도서관에서 무료로 쓸 수 있다면 개인이 굳이 구매하지 않아도 됨
- 오울루의 새로 단장한 중앙도서관 **Saari**도 같은 방식으로 운영됨
  - 방문자는 마이크로필름 리더로 옛 신문을 보고, 악보를 출력하고, 예약한 재봉틀로 청바지를 줄임
  - 같은 공간에서 3D 프린터, 열전사 프레스, 레이저 커터도 제공됨

## 높은 이용률을 만드는 접근성

- 핀란드인의 **55%** 는 최소 월 1회 도서관을 방문함
- 문화교육부 데이터에서 핀란드인은 1년에 **9.1회** 도서관을 이용함
  - 영국 평균은 약 2.5회
  - 미국 평균은 2.4회
  - EU 평균은 약 3.5회
- 오울루대 Noora Hirvonen은 이용률이 낮은 서비스를 비용 절감 목적으로 줄이면 자기충족적 예언이 될 수 있다고 봄
  - 개관 시간을 줄이면 방문자가 줄어듦
  - 줄어든 방문 수가 추가 삭감이나 폐쇄의 근거로 쓰일 수 있음
- 서비스가 덜 쓰이는 이유가 가치 부족인지, 접근성 부족인지, 사람들이 알지 못해서인지 구분해야 함
- 이용률은 서비스의 가치뿐 아니라 **가시성**과 **이용 가능성**에도 좌우됨

## 민주주의를 떠받치는 공공 공간

- 핀란드 도서관은 교수, 실업자, 노숙자가 같은 공간을 쓰는 장소이며, 소득이나 배경과 무관하게 지식 접근·만남·공적 토론 참여를 가능하게 함
- 핀란드 도서관법(Finnish Library Act)은 공공도서관이 **민주주의**, 표현의 자유, 적극적 시민성을 증진해야 한다고 규정함
  - 일부 다른 북유럽 국가도 비슷한 정책을 갖고 있음
- 핀란드는 2025년 공공도서관에 약 **€371m**을 지출함
  - 1인당 €65.78
  - 영국은 1인당 평균 £10
  - 미국의 총 공공도서관 지출은 $15.2bn, 1인당 $45
- Finnish National Digital Support Model의 파트너로서 많은 핀란드 도서관은 다양한 배경의 시민이 디지털 복지 시스템에 접근하도록 지원함
  - 세금 서비스, 은행 계정, 연금 포털, 디지털 건강 기록 같은 온라인 행정 이용을 도와줌
  - 이력서와 구직 신청서 작성도 지원함
- 핀란드 도서관 연구에서 도서관은 중요한 **포용 인프라**로 평가됨

## 비용 대비 가치와 신뢰

- 전 세계 38개 연구를 분석한 결과, 공공도서관은 투자 1달러당 3~5달러의 가치를 돌려주는 것으로 나타남
- 직접적 이익에는 구매 대신 대여하면서 생기는 절감과 구직자 지원이 포함됨
- 간접적 이익은 문해력, 디지털 역량, 고용 가능성, 커뮤니티 웰빙 개선으로 이어짐
- 핀란드 지역 당국의 연례 평가에서 도서관은 가장 높게 평가되는 공공 서비스 목록의 상위권에 계속 오름
- 2023년 보고서에서 핀란드인들은 도서관을 신뢰할 수 있는 정보와 디지털 콘텐츠의 출처로 봄
- R David Lankes는 여러 나라에서 공공기관 신뢰가 장기적으로 하락하는 가운데, 도서관과 사서에 대한 신뢰는 매우 높게 남아 있다고 봄
- 도서관은 소비를 요구받지 않고 그냥 머무를 수 있는 몇 안 되는 **공공 공간**임

## 참여 격차를 메우는 장소

- 핀란드인은 공공기관에 대한 신뢰가 여전히 높지만, 많은 시민은 정치적 결정에 실제 영향력이 작다고 느끼는 **참여의 역설**을 겪음
- 핀란드 혁신기금 Sitra의 Elina Eerola는 도서관이 시민과 의사결정자가 만나고 토론에 참여할 수 있는 접근 가능한 공간을 만들 수 있다고 봄
- Sitra의 시범 프로젝트는 도서관에서 커뮤니티 토론과 행사를 열어 시민을 정치인·공공기관과 직접 연결함

## 한 사람의 삶에 남는 도서관

- 핀란드 의회 의원 Nasima Razmyar는 8세 때 아프가니스탄에서 난민으로 핀란드에 왔고, 첫 도서관 카드를 핀란드에서 처음 소유한 물건으로 기억함
- 헬싱키 Käpylä 지역에서 자라며 방과 후 지역 도서관에서 공부했고, 부모가 핀란드어를 하지 못했기 때문에 도서관 직원들이 숙제를 도와준 적도 있음
- Razmyar는 지역 도서관을 **평등**이자 “한 건물 안의 핀란드 복지 시스템 전체”로 기억함
- 자신의 아이들이 도서관에서 책을 고르는 모습을 보며, 이 공간이 자신들에게 속한다는 느낌을 갖는 것이 중요하다고 봄

---

## 17:55 — DOS 게임 F-15 Strike Eagle II 리버싱 프로젝트, DOS 테스트 파일럿 모집
**source**: geeknews · **tags**: #원본 #실행 #파일 #게임 #구성
**url**: https://news.hada.io/topic?id=30695

- 1989년 게임 **F-15 Strike Eagle II**의 원본 바이너리를 리버스 엔지니어링해 C 소스 코드를 재구성하는 프로젝트가 실제 플레이 테스트 단계에 들어감
- `egame`, `end`를 포함한 모든 실행 파일의 코드와 데이터를 C로 옮겼고, 대부분의 어셈블리 전용 코드에는 **C 대체 구현**이 마련됨
- 최신 릴리스 `v0.9.1`은 원본 게임 `451.03` 버전과 **Desert Storm 확장팩**에서 원본 실행 파일을 교체해 테스트할 수 있음
- 새 실행 파일은 설정 화면을 거치지 않고 **MCGA/VGA**, 무음, 조이스틱 없음 상태를 가정하지만 미션 브리핑·비행·디브리핑은 작동해야 함
- 목표가 원본과 같은 버그까지 보존하는 **bug-for-bug 재구성**이므로, 원본에도 있는 동작은 현재 버그 리포트 대상에서 제외됨
...
- 한 달 조금 전만 해도 두 번째 실행 파일 `egame`을 C로 옮기는 데 몇 년이 더 걸릴 것처럼 보였고, 세 번째 실행 파일 `end`도 남아 있었음
- 현재는 다음 단계까지 진행됨
...
- 테스트 대상은 최신 릴리스 [v0.9.1](https://github.com/neuviemeporte/f15se2-re/releases/tag/v0.9.1)임
- 원본 게임 `451.03` 버전과 Desert Storm 확장팩이 있는 게임 폴더에 새 실행 파일을 넣어 원본 실행 파일을 교체하면 됨
  - 교체 전 **백업**이 필요함
  - 새 `f15.exe` 대신 원본 `f15.com`이 실행되지 않도록 원본 `f15.com`을 제거해야 할 수 있음
- 새 실행 파일은 설정 화면으로 들어가지 않음
  - **MCGA/VGA 디스플레이**를 가정함
  - 사운드 없음
  - 조이스틱 없음
- 테스트 중 문제가 있으면 [GitHub 이슈](https://github.com/neuviemeporte/f15se2-re/issues)로 보고할 수 있음
  - 충돌
  - 그래픽 글리치
  - 키 입력 미작동
  - 필요한 경우 dosbox에서 `Ctrl+F5`로 찍은 스크린샷 첨부
  - 문제가 발생하기 전 어떤 행동을 했는지 적으면 재현과 수정에 도움이 됨
- 이 프로젝트는 **bug-for-bug 재구성**을 목표로 하므로 원본 게임에도 있는 동작은 유지해야 함
  - 원본에는 3D 오브젝트가 사라지는 문제, 뒤집힌 상태에서 연료가 없을 때 비행기가 하늘 쪽으로 떨어지는 문제 등이 있음
  - 보고 전 원본에서도 같은 문제가 발생하는지 확인하는 편이 좋음

---

## 18:35 — Linux, 6년·360개 이상 패치 끝에 strncpy API 제거
**source**: geeknews · **tags**: #strncpy #커널 #복사 #종료 #pad
**url**: https://news.hada.io/topic?id=30696
...
- 금요일에 이뤄진 [머지](https://git.kernel.org/pub/scm/linux/kernel/git/torvalds/linux.git/commit/?id=1a3746ccbb0a97bed3c06ccde6b880013b1dddc1)가 strncpy API를 제거함
- 같은 머지에서 마지막 **per-CPU 아키텍처별 strncpy 구현**도 함께 사라짐

## 커널 코드에서 쓸 대체 API

- strncpy 대신 복사 대상과 종료 조건에 맞는 함수를 선택해야 함
  - **strscpy()**: NUL 종료되는 목적지에 사용
  - **strscpy\_pad()**: NUL 종료되는 목적지에 0 패딩이 필요한 경우 사용
  - **strtomem\_pad()**: NUL 종료되지 않는 고정 폭 필드에 사용
  - **memcpy\_and\_pad()**: 명시적 패딩이 있는 제한된 복사에 사용
  - **memcpy()**: 길이를 알고 있는 메모리 복사에 사용

---

## 19:46 — 아이들을 생각하라: 모든 인터넷 트래픽에 Real ID를 강제하는 방법(2023)
**source**: geeknews · **tags**: #RTA #헤더 #사이트 #성인 #방식
**url**: https://news.hada.io/topic?id=30697
...
- 일부 주와 한 국가는 성인 웹사이트 로그인 때 **성인임을 증명하기 위한 주 신분증과 문서 업로드**를 요구하는 중앙화 상업 데이터베이스를 구현하고 있음
- 이 방식이 성인 콘텐츠에 머물지 않고, 인터넷 사용 전반에 **실명과 금융 정보**를 연결하는 인프라로 커질 수 있다는 점이 핵심 우려임
...
- 과거 브라우저와 애드온은 오래된 표준을 통해 **성인 콘텐츠와 사용자 생성 콘텐츠**를 감지할 수 있었음
- 이 방식은 클라이언트 개발자와 웹 서버 운영자에게 비용을 거의 요구하지 않고, 아이가 무엇을 볼 수 있는지 관리하는 책임을 부모에게 두는 구조였음
- [yt-dlp의 예시 코드](https://github.com/yt-dlp/yt-dlp/blob/master/yt_dlp/extractor/common.py#L1425)는 RTA 헤더를 찾는 구현 사례로 제시됨
- ### ICRA PICS Headers

  - **ICRA PICS**는 아이들에게 더 안전한 웹을 만들기 위한 첫 시도였음
  - 일부 브라우저, 서드파티 도구, 웹 서버가 이를 채택함
  - 사이트에 어떤 유형의 콘텐츠가 있는지 상세히 기술하는 헤더를 웹 폼으로 생성해야 해 마찰이 컸고, 채택은 정체됨
- ### RTA Header

  - [**RTA Header**](https://www.rtalabel.org/index.php?content=howtofaq#single)는 더 단순한 방식으로 더 널리 쓰인 두 번째 시도였음
...
- 글은 다음 흐름을 **이론과 최선의 추정**으로 구분함
  ...
  - 5단계: 추적 프레임워크가 생긴 뒤 Facebook, X, Instagram 같은 **소셜미디어**에도 요구됨
  ...
  - 7단계: 더 큰 수익과 **대규모 추적**이 발생함
- 이런 데이터가 한곳에 모이면 인터넷상의 악의적 행위자를 끌어들일 수 있음
- S3 버킷에서 실수로 유출되거나 데이터가 판매되지 않을 것이라는 보장은 냉소적으로 다뤄짐

## 전역 집행과 수익화 가능성

- 전역 집행은 이론적으로 모든 웹 브라우저가 [**Web Environment Integrity(WEI)**](https://www.eff.org/deeplinks/2023/08/your-computer-should-say-what-you-tell-it-say-1) 서명이 없는 웹페이지를 차단하는 방식으로 가능함
- 이 흐름은 **신용카드, 주 신분증, TPM 모듈**과 연결될 수 있음
...
- 벤더와 OS 업데이트가 **Secure Boot**를 잠가, 서드파티 연령·ID 검증에 참여하지 않는 OS를 막을 수 있다는 우려도 포함됨

## 사회적 영향에 대한 우려

- 발언이 문제가 될 때 시민이 벌금이나 신상 공개를 당할 수 있다고 봄
  - [PayPal이 잘못된 정보에 대해 사용자에게 2,500달러 벌금을 부과하겠다고 했다가 즉시 철회한 사례](https://fortune.com/2022/10/10/paypal-users-fine-misinformation-aup-error-confusion/)가 예로 제시됨
- 사람들이 자신의 신념을 말할 때 **물리적·금융적 보복** 위험이 커지고, 그 결과 자기검열이 늘어날 수 있음
- 누군가가 생각하는 [**Utopia**](https://www.youtube.com/watch?v=vJYaXy5mmA8)가 사회에 강요될 수 있다는 우려도 있음
...
- 청소년이 헤더를 우회하는 상황이, 중앙화 데이터베이스를 쓰기 위해 **신용카드를 훔치거나 ID를 위조하고 범죄 이력으로 삶을 시작하는 상황**보다 낫다는 입장임
...
- 사람들은 주 및 연방 대표에게 연락해, 더 단순하고 **프라이버시 침해가 적은 연령 인증 방식**을 요구해야 함
...
- 넷째, **2034년 기준 13세 미만 모든 아동**에게 부모 통제가 활성화되도록 법제화함
...
- 개인식별정보를 업로드해야 한다면, 직접·간접 관련 데이터센터는 [**PCI DSS**](https://www.pcisecuritystandards.org/standards/)와 [**Fedramp**](https://www.fedramp.gov/)를 합친 것보다 더 엄격한 기술·감사 요구사항을 따라야 함
  - IoT 기기, 개발자 노트북, DEV/QA, 성능, 스테이징, 프로덕션까지 **모든 것**이 범위에 들어가야 함
  - 이것이 너무 어렵다면 개인식별정보를 건드리지 말고 RTA·성인 헤더를 쓰라는 결론임

## CTO와 CSO에게 주는 권고

- 법이 된 뒤 역풍이 불고 실패할 때까지 기다리지 말고, 사이트에 **RTA 헤더**를 구현해야 함
- 브라우저에는 헤더 검사를 넣어 부모와 아이를 보호할 수 있어야 함
- 기업은 이러한 구현을 통해 다른 곳보다 앞서 있다고 말할 수 있음

---

## 21:37 — Linux의 epoll과 io_uring 비교
**source**: geeknews · **tags**: #epoll #io #uring #완료 #syscall
**url**: https://news.hada.io/topic?id=30698
...
- 이후 epoll의 한계 때문에 **io\_uring**으로 전환했고, 프로젝트를 처음부터 다시 작성하게 됨
...
## io\_uring: 완료 모델과 공유 링 버퍼

- io\_uring은 epoll이 Linux kernel에 들어간 지 약 17년 뒤인 2019년에 등장했고, kernel v5.1+에서 지원됨
...
- ### io\_uring 예제
  ...
  - io\_uring 예제에는 별도 **준비 상태 확인**이 없고, 완료 시점에 따로 `read()`를 호출하지 않음
  - 단순화를 위해 두 예제에는 중요한 예외 처리가 빠져 있음
  - `stdin`에 데이터가 없으면 영원히 블록될 수 있음
  - io\_uring 예제는 제출 큐가 가득 찼을 때 `io_uring_get_sqe()`가 `NULL`을 반환하는 경우를 검사하지 않음

## io\_uring을 쓸 때의 추가 조건
...
- io\_uring의 오류는 동기 syscall의 직접 반환값이 아니라 완료 큐 엔트리의 `res` 필드로 비동기적으로 돌아옴
  - 오류 처리는 `cqe->res`를 통해 해야 함

## 최신 Linux 서버에서의 선택

- epoll은 I/O 가능 시점 통지와 별도 syscall 호출에 기반한 오래된 Linux 비동기 I/O 방식임
- io\_uring은 최신 Linux에서 **완료 기반 모델**과 배치 제출·완료 처리를 제공함
- 현대 Linux 서버에서 처음부터 새 프로젝트를 만든다면 **io\_uring**을 선택하는 쪽이 자연스러움
- 오래된 시스템 지원을 합리적인 시점에 중단할 수 있다면, kernel v5.1+ 환경에서는 epoll을 고를 이유가 많지 않음

---

## 21:47 — Show HN: TownSquare, 웹사이트를 위한 작은 존재감 레이어
**source**: geeknews · **tags**: #사이트 #데모 #방문자 #사람 #TownSquare
**url**: https://news.hada.io/topic?id=30699
