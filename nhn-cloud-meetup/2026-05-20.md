...
## 08:29 — MySQL 3분 vs ClickHouse 0.3초 — 같은 쿼리입니다
**source**: nhn-cloud-meetup · **tags**: #ClickHouse #MySQL #데이터 #쿼리 #집계
**url**: https://meetup.nhncloud.com/posts/414

[![NHN Cloud\_meetup banner\_CH\_202605\_900.png](https://image.toast.com/aaaadh/real/2026/techblog/NHN%20Cloudmeetup%20bannerCH202605900.png)](https://www.nhncloud.com/kr)
## 들어가며
...
![NHN Cloud\_meetup diagram\_clickHouse\_01.gif](https://image.toast.com/aaaadh/real/2026/techblog/NHN%20Cloudmeetup%20diagramclickHouse01.gif)
...
![NHN Cloud\_meetup diagram\_clickHouse\_02.gif](https://image.toast.com/aaaadh/real/2026/techblog/NHN%20Cloudmeetup%20diagramclickHouse02.gif)
(source: [https://clickhouse.com/docs/intro](https://clickhouse.com/docs/intro))
  
![NHN Cloud\_meetup diagram\_clickHouse\_03\_900.png](https://image.toast.com/aaaadh/real/2026/techblog/NHN%20Cloudmeetup%20diagramclickHouse03900.png)
...
SELECT DATE(created\_at) AS day, COUNT(\*) AS cnt, SUM(amount) AS total
FROM orders
WHERE created\_at BETWEEN '2024-01-01' AND '2024-12-31'
GROUP BY day
ORDER BY day;
...
```
-- 서비스별 + 날짜별 + 타입별 3중 집계
SELECT service\_id, DATE(created\_at), event\_type, SUM(amount)
FROM transaction\_log
WHERE created\_at BETWEEN '2024-01-01' AND '2024-12-31'
GROUP BY service\_id, DATE(created\_at), event\_type;
```
...
### 1. 단건 조회/단건 INSERT
```
SELECT \* FROM users WHERE user\_id = 12345;
```
...
```
BEGIN;
UPDATE orders SET status = 'paid' WHERE order\_id = 999;
UPDATE stock SET quantity = quantity - 1 WHERE item\_id = 1;
COMMIT;
```
...
### 3. 빈번한 UPDATE/DELETE
```
UPDATE orders SET status = 'cancel' WHERE order\_id = 999;
DELETE FROM logs WHERE created\_at < '2023-01-01';
```
...
PRIMARY KEY (`id`),
INDEX idx\_created\_at\_service\_event (created\_at, service\_id, event\_type)
...
ENGINE = MergeTree()
ORDER BY (created\_at, service\_id, event\_type);
...
```
SELECT DATE\_FORMAT(created\_at, '%Y-%m'), SUM(amount)
FROM events
...
```
SELECT service\_id, DATE(created\_at), event\_type, COUNT(\*), SUM(amount)
FROM events
WHERE created\_at BETWEEN '2024-01-01' AND '2024-12-31'
GROUP BY 1, 2, 3;
```
...
```
SELECT event\_type, status,
COUNT(\*), COUNT(DISTINCT user\_id), SUM(IF(amount > 50000, 1, 0))
FROM events
WHERE created\_at BETWEEN '2024-06-01' AND '2024-12-31'
GROUP BY event\_type, status;
```
| 항목 | MySQL | ClickHouse | 차이 |
| --- | --- | --- | --- |
...
### 1. Notification Hub
NHN Cloud의 [Notification Hub](https://www.nhncloud.com/kr/service/notification/notification-hub)는 SMS, Email, Push 등 다양한 채널로 메시지를 발송·관리하는 통합 메시징 서비스입니다.
...
### 3. Resource Watcher
NHN Cloud의 [Resource Watcher](https://www.nhncloud.com/kr/service/governance-audit/resource-watcher)는 NHN Cloud의 로그 플랫폼 개발팀에서 직접 ClickHouse를 도입하여 운영 중인 사례입니다. MySQL에서 반복적으로 발생하던 슬로 쿼리 문제를 ClickHouse로 해결한 경험을 공유해 주셨습니다.
...
>
> **2\) 배치 조회와 서비스 트래픽의 DB 레벨 격리 -** 배치와 서비스가 각각 다른 DB를 바라보도록 분리하여 서로 영향을 주지 않고 독립적으로 동작합니다.
>
> **3\) 쿼리 구조 개선으로 조회 성능 향상 -** LIMIT/OFFSET 쿼리 수행 시 발생하던 성능 문제가 개선되었습니다.
  
## 타사 ClickHouse 사용 사례
최근에는 많은 기업들이 다양한 목적으로 ClickHouse를 도입해 사용하고 있습니다. 대표적인 사례를 소개합니다.
...
## 마치며
지금까지 ClickHouse가 무엇인지, MySQL과 대비하여 구조적으로 어떻게 다르고 왜 빠른지 알아보았습니다. 나아가 설계 철학부터 다른 두 데이터베이스를 각각 어떤 케이스에 활용하면 적합한지 살펴보았고, 실제로 ClickHouse를 활용하는 사례들을 소개했습니다.
MySQL의 집계 쿼리 성능 한계로 슬로 쿼리와 타임아웃을 자주 마주하시는 분들, ClickHouse에 관심이 있으시거나 도입을 고려하시는 분들 등 많은 분들께 도움이 되길 바라며 글을 마치겠습니다. 긴 글을 읽어 주셔서 감사합니다.
[![NHN Cloud\_meetup banner\_footer\_202509.png](https://image.toast.com/aaaadh/real/2026/techblog/NHN%20Cloudmeetup%20bannerfooter202509.png)](https://www.nhncloud.com/kr)

---

## 08:29 — 인증서 톺아보기 시리즈 3: 제로 트러스트와 mTLS
**source**: nhn-cloud-meetup · **tags**: #인증서 #서비스 #mTLS #CA #클라이언트
**url**: https://meetup.nhncloud.com/posts/413

[![NHN Cloud\_meetup banner\_certificate\_202604\_900.png](https://image.toast.com/aaaadh/real/2026/techblog/NHN%20Cloudmeetup%20bannercertificate202604900.png)](https://www.nhncloud.com/kr)
## 들어가며
인증서 톺아보기 두 번째 시리즈, [유효기간 감소와 발급/갱신 자동화](https://meetup.nhncloud.com/posts/411)에서 우리는 서버가 클라이언트에게 "저는 진짜 example.com입니다"라고 증명하는 일반적인 단방향 TLS에 대해 알아봤습니다.
...
![NHN Cloud\_meetup diagram\_certificate3\_01\_900.png](https://image.toast.com/aaaadh/real/2026/techblog/NHN%20Cloudmeetup%20diagramcertificate301900.png)
...
![NHN Cloud\_meetup diagram\_certificate3\_02\_900.png](https://image.toast.com/aaaadh/real/2026/techblog/NHN%20Cloudmeetup%20diagramcertificate302900.png)
### mTLS의 주요 사용 사례
\* \*\*마이크로서비스 간 통신\*\*
...
```
![NHN Cloud\_meetup diagram\_certificate3\_03\_900.png](https://image.toast.com/aaaadh/real/2026/techblog/NHN%20Cloudmeetup%20diagramcertificate303900.png)
...
### 인증서 라이프사이클
![NHN Cloud\_meetup diagram\_certificate3\_04\_900.png](https://image.toast.com/aaaadh/real/2026/techblog/NHN%20Cloudmeetup%20diagramcertificate304900.png)
...
![NHN Cloud\_meetup diagram\_certificate3\_05\_900.png](https://image.toast.com/aaaadh/real/2026/techblog/NHN%20Cloudmeetup%20diagramcertificate305900.png)
...
[![NHN Cloud\_meetup banner\_footer\_skyblue\_202604\_900.png](https://image.toast.com/aaaadh/real/2026/techblog/NHN%20Cloudmeetup%20bannerfooterskyblue202604900.png)](https://www.nhncloud.com/kr)

---

## 08:29 — Artifact Driven Development: 과정을 보지 않기로 했다
**source**: nhn-cloud-meetup · **tags**: #에이전트 #Plan #TC #과정 #결과
**url**: https://meetup.nhncloud.com/posts/412

[![NHN Cloud\_meetup banner\_ADD\_202604\_900.png](https://image.toast.com/aaaadh/real/2026/techblog/NHN%20Cloudmeetup%20bannerADD202604900.png)](https://www.nhncloud.com/kr)
## 들어가며
...
![ADD\_2\_900.png](https://image.toast.com/aaaadh/real/2026/techblog/ADD2900.png)
...
![ADD\_3\_900.png](https://image.toast.com/aaaadh/real/2026/techblog/ADD3900.png)
...
이 글이 저와 같은 고민을 하는 동료 개발자 분들께 도움이 되길 바라며 마치겠습니다. 긴 글을 읽어 주셔서 감사합니다.
### 참고 자료
...
[![NHN Cloud\_meetup banner\_footer\_gray\_202604\_900.png](https://image.toast.com/aaaadh/real/2026/techblog/NHN%20Cloudmeetup%20bannerfootergray202604900%281%29.png)](https://www.nhncloud.com/kr)

---

## 08:29 — 인증서 톺아보기 시리즈 2: 유효기간 감소와 발급/갱신 자동화
**source**: nhn-cloud-meetup · **tags**: #인증서 #자동 #ACME #DNS #갱신
**url**: https://meetup.nhncloud.com/posts/411

[![NHN Cloud\_meetup banner\_certificate\_202603\_900.png](https://image.toast.com/aaaadh/real/2026/techblog/NHN%20Cloudmeetup%20bannercertificate202603900.png)](https://www.nhncloud.com/kr)
## 들어가며
...
### 점점 짧아지는 유효기간
![certificate2\_1.png](https://image.toast.com/aaaadh/real/2026/techblog/certificate21.png)
### 왜 짧아지는가?
#### 1\. 보안 향상
![NHN Cloud\_meetup\_certificate2\_2\_900.png](https://image.toast.com/aaaadh/real/2026/techblog/NHN%20Cloudmeetupcertificate22900.png)
...
![NHN Cloud\_meetup\_certificate2\_3\_900.png](https://image.toast.com/aaaadh/real/2026/techblog/NHN%20Cloudmeetupcertificate23900.png)
## 자동화 - ACME 프로토콜
...
![NHN Cloud\_meetup\_certificate2\_4\_900.png](https://image.toast.com/aaaadh/real/2026/techblog/NHN%20Cloudmeetupcertificate24900.png)
##### 1\. HTTP-01 Challenge
```
동작 방식
──────────
...
\* 비교적 무거움
\*\*자동 갱신 메커니즘\*\*
\* Certbot 설치 시 \*\*systemd timer\*\* 자동 설정(`certbot.timer` - OS마다 동작이 달라서 확인 필요 **"직접 확인 필수"**)
...
> \* 플러그인은 별도 설치 필요: `sudo apt install python3-certbot-dns-`
...
# 실전에서는 DNS API를 사용한 자동화 방법을 권장합니다(예: --dns dns\_aws, --dns dns\_cf)
acme.sh --issue --dns \
...
\*\*인증서를 수동으로 갱신하던 시\*\*는 이제 역사가 되어가고 있습니다.
...
[![NHN Cloud\_meetup banner\_footer\_mint\_202601.png](https://image.toast.com/aaaadh/real/2026/techblog/NHN%20Cloudmeetup%20bannerfootermint202601%283%29.png)](https://www.nhncloud.com/kr)

---

## 08:29 — Claude Code로 Figma 디자인 시스템을 C++ 코드로 변환하기
**source**: nhn-cloud-meetup · **tags**: #디자인 #코드 #토큰 #constexpr #색상
**url**: https://meetup.nhncloud.com/posts/410

[![NHN Cloud\_meetup banner\_designSystem\_202603\_rv.png](https://image.toast.com/aaaadh/real/2026/techblog/NHN%20Cloudmeetup%20bannerdesignSystem202603rv.png)](https://www.nhncloud.com/kr)
안녕하세요.
제로 트러스트 보안 모델을 기반으로 NHN Cloud 리소스에 안전하게 접속할 수 있도록 지원하는 [Cloud Access](https://www.nhncloud.com/kr/service/security/cloud-access)의 Windows 클라이언트를 개발하고 있는 앱서비스보안팀 정원희입니다.
...
![CC\_designsystem\_1\_900.png](https://image.toast.com/aaaadh/real/2026/techblog/CCdesignsystem1900.png)
...
```cpp
#define COLOR\_ACCENT RGB(0x12, 0x5D, 0xE6)
#define BLUE\_BTN\_BG RGB(18, 93, 230)
...
![CC\_designsystem\_2.png](https://image.toast.com/aaaadh/real/2026/techblog/CCdesignsystem2.png)
...
```cpp
namespace sizePx {
inline constexpr int heading\_lg = 16; // Modal title
inline constexpr int body\_sm = 13; // Button S/M, general text
inline constexpr int caption\_md = 12; // Footer, Tooltip
}
namespace weight {
inline constexpr DWORD regular = 400; // FW\_NORMAL
inline constexpr DWORD medium = 500;
inline constexpr DWORD bold = 700; // FW\_BOLD
}
struct TextStyle {
int sizePx;
DWORD weight;
...
![CC\_designsystem\_3\_900.png](https://image.toast.com/aaaadh/real/2026/techblog/CCdesignsystem3900.png)
...
\* [NHN Cloud, Cloud Access](https://www.nhncloud.com/kr/service/security/cloud-access)
\* [Claude API Docs, 모델 기능-PDF 지원](https://platform.claude.com/docs/ko/build-with-claude/pdf-support)
[![NHN Cloud\_meetup banner\_footer\_blue\_202509.png](https://image.toast.com/aaaadh/real/2026/techblog/NHN%20Cloudmeetup%20bannerfooterblue202509.png)](https://www.nhncloud.com/kr)

---

## 08:29 — 인증서 톺아보기 시리즈 1: 인증서란? 디지털 신뢰의 시작
**source**: nhn-cloud-meetup · **tags**: #인증서 #CA #개인 #PEM #DER
**url**: https://meetup.nhncloud.com/posts/409

[![NHN Cloud\_meetup banner\_certificate\_900.png](https://image.toast.com/aaaadh/real/2026/techblog/NHN%20Cloudmeetup%20bannercertificate900.png)](https://www.nhncloud.com/kr)
## 들어가며
...
### 인증서의 핵심 구성 요소
![certificate\_1\_900.png](https://image.toast.com/aaaadh/real/2026/techblog/certificate1900.png)
...
![certificate\_2\_900.png](https://image.toast.com/aaaadh/real/2026/techblog/certificate2900.png)
\*\*서명 생성 과정\*\*
```
...
![certificate\_3\_900.png](https://image.toast.com/aaaadh/real/2026/techblog/certificate3900.png)
## 인증 기관(CA)과 신뢰 체인
### CA(Certificate Authority)의 역할
CA는 인증서를 발급하고 그 진위를 보증하는 \*\*디지털 공증인\*\*입니다.
![certificate\_4\_900.png](https://image.toast.com/aaaadh/real/2026/techblog/certificate4900.png)
### 신뢰 체인(Chain of Trust) 검증
1) 브라우저가 서버 인증서 수신
CN: \*.nhncloud.com
Issuer: Sectigo RSA Organization Validation Secure Server CA
![certificate\_5.png](https://image.toast.com/aaaadh/real/2026/techblog/certificate5.png)
2) Intermediate CA 인증서 확인
Issuer: USERTrust RSA Certification Authority
![certificate\_6.png](https://image.toast.com/aaaadh/real/2026/techblog/certificate6.png)
3) Root CA 확인
→ 브라우저/OS에 사전 설치된 신뢰할 수 있는 Root CA 목록에 있는가?
✓ 있으면 신뢰
✗ 없으면 경고("연결이 비공개로 설정되어 있지 않습니다")
![certificate\_7.png](https://image.toast.com/aaaadh/real/2026/techblog/certificate7.png)
### 주요 CA 업체들
\* Let's Encrypt
\* DigiCert
\* GlobalSign
...
## 참고 자료
### 참고 문헌
\* Sharon Boeyen 외, RFC - [Proposed Standard](https://tools.ietf.org/html/rfc5280), 2008. 5.
\* Qualys SSL Labs, [SSL Server Test](https://www.ssllabs.com/ssltest/)
\* 한국전자인증, [[중요공지] SSL/TLS 인증서 유효기간 단축 확정](https://cert.crosscert.com/%EC%A4%91%EC%9A%94%EA%B3%B5%EC%A7%80-ssl-tls-%EC%9D%B8%EC%A6%9D%EC%84%9C-%EC%9C%A0%ED%9A%A8%EA%B8%B0%EA%B0%84-%EB%8B%A8%EC%B6%95-%ED%99%95%EC%A0%95/), 2025. 4. 18.
### NHN Cloud 인증서 관리 서비스 살펴보기
> \* \*\*[Certificate Manager](https://www.nhncloud.com/kr/service/management/certificate-manager)\*\*: 인증서, 도메인 등 만료 기한이 있는 자산을 관리하고 만료일 알림을 제공하는 서비스
> \* \*\*[Private CA](https://www.nhncloud.com/kr/service/management/private-ca)\*\*: 조직 내·외부에서 사용하는 사설 인증서를 발급부터 폐기까지 자동화 및 표준화하는 서비스
[![NHN Cloud\_meetup banner\_footer\_blue\_202412\_900.png](https://image.toast.com/aaaadh/real/2026/techblog/NHN%20Cloudmeetup%20bannerfooterblue202412900.png)](https://www.nhncloud.com/kr)

---

## 08:29 — 읽지 않는 코드의 시대
**source**: nhn-cloud-meetup · **tags**: #코드 #AI #함수 #인간 #프로그래밍
**url**: https://meetup.nhncloud.com/posts/408

[![NHN Cloud\_meetup banner\_coding\_202602\_900.png](https://image.toast.com/aaaadh/real/2026/techblog/NHN%20Cloudmeetup%20bannercoding202602900.png)](https://www.nhncloud.com/kr)
...
---
> 본 콘텐츠는 작성자가 사내 게시판에 공유한 글을 가공한 것으로, 작성자의 의도와 맥락을 충실히 전달하기 위해 원문의 문장 스타일을 그대로 유지하였습니다.
[![NHN Cloud\_meetup banner\_footer\_202507-01.png](https://image.toast.com/aaaadh/real/2026/techblog/NHN%20Cloudmeetup%20bannerfooter20250701.png)](https://www.nhncloud.com/kr)

---

## 08:29 — 2025 프론트엔드 뉴스 한 방에 몰아 보기
**source**: nhn-cloud-meetup · **tags**: #React #to #AI #TypeScript #Next.js
**url**: https://meetup.nhncloud.com/posts/407

[![NHN Cloud\_meetup banner\_frontendnews2025\_202601\_1400.png](https://image.toast.com/aaaadh/real/2026/techblog/NHN%20Cloudmeetup%20bannerfrontendnews20252026011400%282%29.png)](https://www.nhncloud.com/?utm\_source=meetup&utm\_medium=post&utm\_campaign=branding&utm\_content=0130\_frontend&utm\_term=top)
...
작년 말 v19 릴리스 이후 `action`, `useActionState`, `useOptimistic`, `` 같은 기능이 추가됐습니다. Form Submit, Loading, Error 등 웹 애플리케이션이 늘 겪는 문제를 React 기본 기능으로 끌어당긴 모양새입니다. 다만 동시성이 기본값이 될수록 개발자도 새로운 규칙을 익혀야 합니다. 코드는 더 선언적으로 보이지만, 어떤 상태를 어떤 방식으로 업데이트할지 조직 내 합의도 필요해질 것 같습니다.
...
\* [The Mutability \& Aliasing Model](https://github.com/facebook/react/blob/156b7a96f5669470182ad226306184576d6f150f/compiler/packages/babel-plugin-react-compiler/src/Inference/MUTABILITY\_ALIASING\_MODEL.md)
## Next.js의 보안 이슈와 복잡성
...
\* [The `` element can now be customized with CSS](https://developer.chrome.com/blog/a-customizable-select?hl=en)
...
\* [MDN - Masonry Layout](https://developer.mozilla.org/en-US/docs/Web/CSS/Guides/Grid\_layout)
### 색상과 스타일링
...
[![NHN Cloud\_meetup banner\_footer\_mint\_202601.png](https://image.toast.com/aaaadh/real/2026/techblog/NHN%20Cloudmeetup%20bannerfootermint202601%282%29.png)](https://www.nhncloud.com/?utm\_source=meetup&utm\_medium=post&utm\_campaign=branding&utm\_content=0130\_frontend&utm\_term=bottom)

---

## 08:29 — 수직 중앙 정렬한 텍스트가 치우쳐 보이는 이유
**source**: nhn-cloud-meetup · **tags**: #크기 #폰트 #px #font #정렬
**url**: https://meetup.nhncloud.com/posts/397

[![NHN Cloud\_meetup banner\_fontAlign\_202511.png](https://image.toast.com/aaaadh/real/2025/techblog/NHN%20Cloudmeetup%20bannerfontAlign202511.png)](https://www.nhncloud.com/kr)
...
| 요소 | 설명 |
| -------------- | ----------------------------------------------------------------------------------------------------------- |
| \*\*EM Square\*\* | 글꼴의 모든 문자가 설계되는 가상의 좌표 공간  
모든 글자는 이 가상의 정사각형 안에서 상대 좌표로 설계된다. |
...
| ----------------------------------- | ----------------------------------- | ------------------------------------- | ------------------------------------- | ------------------------------------- | -------------------------------------- |
|![text5\_chrome.png](https://image.toast.com/aaaadh/real/2025/techblog/text5chrome.png)|![text5\_chrome2x.png](https://image.toast.com/aaaadh/real/2025/techblog/text5chrome2x.png)|![text5\_safari.png](https://image.toast.com/aaaadh/real/2025/techblog/text5safari.png)|![text5\_safari2.png](https://image.toast.com/aaaadh/real/2025/techblog/text5safari2.png)|![text5\_firefox.png](https://image.toast.com/aaaadh/real/2025/techblog/text5firefox.png)|![text5\_firefox2.png](https://image.toast.com/aaaadh/real/2025/techblog/text5firefox2.png)|
...
| Chrome | Chrome(2x) |
| ------------------------------------------------------------------ | ------------------------------------------------------------------ |
|![text6\_chrome.png](https://image.toast.com/aaaadh/real/2025/techblog/text6chrome.png) |![text6\_chrome2.png](https://image.toast.com/aaaadh/real/2025/techblog/text6chrome2.png) |
|![text7\_chrome.png](https://image.toast.com/aaaadh/real/2025/techblog/text7chrome.png)| ![text7\_chrome2.png](https://image.toast.com/aaaadh/real/2025/techblog/text7chrome2.png) |
...
### \*\*실용적인 대응 방법\*\*
1. \*\*CSS transform 활용하기\*\*[](https://codepen.io/jajugoguma/pen/OPVVwgR?editors=1100)
...
```
2. \*\*폰트별 보정 값 적용하기\*\*[](https://codepen.io/jajugoguma/pen/WbvvKzZ?editors=1400)
...
4. \*\*텍스트 박스에 패딩/마진 추가하기\*\*[](https://codepen.io/jajugoguma/pen/pvJJZQK?editors=1400%3C/sup%3E)
...
```
5. \*\*브라우저 환경 감지 및 조건부 스타일 적용하기\*\*[](https://codepen.io/jajugoguma/pen/EajjpJB)
...
[![NHN Cloud\_meetup banner\_footer\_202511.png](https://image.toast.com/aaaadh/alpha/2025/techblog/NHN%20Cloudmeetup%20bannerfooter202511.png)](https://www.nhncloud.com/kr)

---

## 08:29 — AI 코딩 도구 보안 가이드: 실제 사고 사례와 DevContainer 격리 환경 구축 방법
**source**: nhn-cloud-meetup · **tags**: #설정 #AI #Code #보안 #Claude
**url**: https://meetup.nhncloud.com/posts/396

[![NHN Cloud\_meetup banner2\_DevContainer\_202509.png](https://image.toast.com/aaaadh/real/2025/techblog/NHN%20Cloudmeetup%20banner2DevContainer202509.png)](https://www.nhncloud.com/kr)
## 들어가며
...
![devContainer\_1.png](https://image.toast.com/aaaadh/real/2025/techblog/devContainer1.png)
(Source: [Jason Lemkin X(구 Twitter) 게시글](https://x.com/jasonlk/status/1946076292736221267)) 
...
![devContainer\_2.png](https://image.toast.com/aaaadh/real/2025/techblog/devContainer2.png)
### 위험 요소 분석
...
### 보안 아키텍처
![devContainer\_3.png](https://image.toast.com/aaaadh/real/2025/techblog/devContainer3.png)
```
호스트 시스템(보호 영역)
...
"API\_BASE\_URL": "http://localhost:8080",
"CUSTOM\_VAR": "your-value"
}
```
\*\*⑥ 패키지 설치 순서\*\*
```bash
...
# GitHub IP 범위 동적 추가
gh\_ranges=$(curl -s https://api.github.com/meta)
echo "$gh\_ranges" | jq -r '(.web + .api + .git)[]' | \
...
for domain in "${ADDITIONAL\_DOMAINS[@]}"; do
if [ -n "$domain" ]; then
...
HOST\_IP=$(ip route | grep default | cut -d" " -f3)
if [ -z "$HOST\_IP" ]; then
echo "ERROR: Failed to detect host IP"
exit 1
fi
HOST\_NETWORK=$(echo "$HOST\_IP" | sed "s/\.[0-9]\*$/.0\/24/")
echo "Host network detected as: $HOST\_NETWORK"
# 나머지 iptables 규칙 설정
iptables -A INPUT -s "$HOST\_NETWORK" -j ACCEPT
iptables -A OUTPUT -d "$HOST\_NETWORK" -j ACCEPT
# 기본 정책을 DROP으로 먼저 설정
...
```
#### \*\*커스터마이징 방법\*\*
\* `ADDITIONAL\_DOMAINS` 배열에 허용하고 싶은 도메인 추가
\* `ADDITIONAL\_IPS` 배열에 특정 IP 주소 직접 추가
\* 주석 처리된 예시를 참고하여 필요한 항목만 활성화
  
### 5. VS Code에서 환경 실행
![devContainer\_4.png](https://image.toast.com/aaaadh/real/2025/techblog/devContainer4.png)
...
#### \*\*실행 단계\*\*
![devContainer\_5.png](https://image.toast.com/aaaadh/real/2025/techblog/devContainer5.png)
① VS Code에서 프로젝트 폴더 열기
② 좌하단 \*\*Open a Remote Window\*\* 버튼 클릭
![devContainer\_6.png](https://image.toast.com/aaaadh/real/2025/techblog/devContainer6.png)
③ \*\*Reopen in Container\*\* 선택
![devContainer\_7.png](https://image.toast.com/aaaadh/real/2025/techblog/devContainer7.png)
④ 컨테이너 빌드 완료 및 방화벽 정상작동 확인 후 터미널에서 `claude --dangerously-skip-permissions` 실행
![devContainer\_8.png](https://image.toast.com/aaaadh/real/2025/techblog/devContainer8.png)
⑤ 필요시 Gemini CLI도 확인
![devContainer\_9.png](https://image.toast.com/aaaadh/real/2025/techblog/devContainer9.png)
⑥ 다시 로컬로 돌아가려면 \*\*Reopen Folder Locally\*\* 버튼 클릭
![devContainer\_10.png](https://image.toast.com/aaaadh/real/2025/techblog/devContainer10.png)
...
[![NHN Cloud\_meetup banner\_footer\_202509.png](https://image.toast.com/aaaadh/real/2025/techblog/NHN%20Cloudmeetup%20bannerfooter202509.png)](https://www.nhncloud.com/kr)
